...
class _tanque: public _triangulos3D
{
       public:
              ...
              void seleccion(float c); //para pintar en el buffer trasero
              //float color_seleccion; //para marcar los objetos seleccionados
              float r_sel,g_sel,b_sel;
              int activo[3];  //para indicar qué pieza está seleccionada
              int cambiar[3]; //indica si una pieza ha sido previamente seleccionada
...
class _flexo: public _triangulos3D
{
       public:
              ...
              float giroY_brazo1;
              float animacion_brazo1_Y = 0.1;//0.1;
              float velocidad_brazo1_Y = 0.1;//0.1;
              
              float giroZ_brazo1;
              float giroZ_brazo1_min;
              float giroZ_brazo1_max;
              float animacion_brazo1_Z = 0.1;//0.01;
              float velocidad_brazo1_Z = 0.1;//0.01;

              float giroZ_brazo2;
              float giroZ_brazo2_min;
              float giroZ_brazo2_max;
              float animacion_brazo2_Z = 0.1;//0.01;
              float velocidad_brazo2_Z = 0.1;//0.01;

              float giroX_cabeza;
              float giroX_cabeza_max;
              float giroX_cabeza_min;
              float animacion_cabeza_X = 0.1;//0.005;
              float velocidad_cabeza_X = 0.1;//0.005;

              void seleccion(float c); //para pintar en el buffer trasero
              //float color_seleccion; //para marcar los objetos seleccionados
              float r_sel,g_sel,b_sel;
              int activo[4];  //para indicar qué pieza está seleccionada
              int cambiar[4]; //indica si una pieza ha sido previamente seleccionada
       protected:
              _base base;
              _brazo1 brazo1;
              _brazo2 brazo2; 
