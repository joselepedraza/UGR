...
class Gausiana{
	...
	public:
		Gausiana(double espe, double desv){
			esperanza = espe;
			desviacion = desv;
		}	
		
		double CalculaG(double abcisa){
			double const MINEXP = -0.5, UNO = 1.0, DOS = 2.0, PI = 3.141516;
	
			double resultado,frac,expon;
	
			frac = UNO /(desviacion * sqrt(DOS * PI));
			expon = exp(MINEXP*(pow((abcisa-esperanza)/desviacion, DOS)));
			resultado = frac * expon;
	
			return resultado;
		}
		
		double AreaCDF(double abcisa){
			...
			t=1/(1+B0 * abcisa);
			resultado_area = 1 - CalculaG(abcisa)*(B1*t + B2*Potencia(t,1) + B3*Potencia(t,3) + B4*Potencia(t,4) + B5*Potencia(t,5));
				...
				do{
					...
					if(op1=='R'){
							...
							for(double i = minimo; i <= maximo; i = i + incremento){
								ordenada = gausiana.CalculaG(i);
								area_gausiana = gausiana.AreaCDF(i);
								cout << "\n\tf(" <<i<< ") = " << ordenada << "\n\t";
