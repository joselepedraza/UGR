...
*/
   ...
   while (caracter != TERMINADOR && total_introducidos < capacidad_secuencia){
	...
	mas_repetido = secuencia.Moda();
	veces = secuencia.Frecuencia(mas_repetido);
	cout << "\n\tEl caracter más repetido es " << mas_repetido << " y aparece " << veces << " veces." << endl;
	
	//monstrar secuencia introducida (sin espacios)
	cout << "\n" << secuencia.ToString();
}
