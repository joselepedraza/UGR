...
public:
   ...
   //ELIMINA OCURRENCIAS(ineficiente)
	...
	/*
	//ELIMINA BLOQUE(ineficiente) -.-
	void EliminaBloqueINEF(int pos_izq, int pos_drc){
		for (int i = 0; i < total_utilizados; i++){
			for(int i = ;  i < ; i++){
				//if(i >= pos_izq && i <= pos_drc){	
					Elimina(i);
				//}
	...
	*/
	...
	//ELIMINA BLOQUE (eficiente)
	void EliminaBloque(int pos_izq, int pos_drc){
		int num_a_borrar = pos_drc - pos_izq +1;
	
		for(int i = 0; i < total_utilizados; i++){
			if(i >= pos_drc+1 /*&& i < total_utilizados*/){
...
*/
   ...
   while (caracter != TERMINADOR && total_introducidos < capacidad_secuencia){
 	...
 	int pos_izquierda, pos_derecha;
 
 	cout << "Introduce la posicion izquierda: ";
 	pos_izquierda = LeeIntRango(0, secuencia.TotalUtilizados());
 	cout << "Introduce la posicion derecha: ";
 	pos_derecha = LeeIntRango(pos_izquierda, secuencia.TotalUtilizados());
 
 	secuencia.EliminaBloque(pos_izquierda, pos_derecha);
 
 	//monstrar secuencia (sin espacios)
	cout << "\n" << secuencia.ToString();  
}
