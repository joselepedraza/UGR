...
public:
   ...
   //ELIMINA OCURRENCIAS(ineficiente)
	void EliminaOcurrenciasINEF(char a_borrar){
		for (int i = 0; i < total_utilizados; i++){
	...
	void EliminaOcurrencias(char a_borrar){
		...
		for(int pos_lectura = 0; pos_lectura < total_utilizados; pos_lectura++){
			if (vector_privado[pos_lectura] == a_borrar){	
				Elimina(pos_lectura);
				//pos_escritura = pos_lectura -1;	
				pos_escritura--;	//por si hay varios caracteres a_borrar seguidos
			}
			else{
				pos_lectura = pos_escritura;
				pos_lectura++;
				pos_escritura++;
			}
		}	
	...
	*/
	...
	//ELIMINA OCURRENCIAS (eficiente) 	algoritmo bueno
	void EliminaOcurrencias(char a_borrar){
