...
void QuienEsQuien::iniciar_juego(){
	...
	/*INI_TEST*/
	bool respuesta=true;
	//Pruebo con un array lleno de caracteristicas, forzando a que se pase de la longitud del arbol construido
	vector<bool> v;
	v.push_back(1);
	v.push_back(0);
	v.push_back(1);
	...
	v.push_back(0);
	cout<<"**************************"<<endl;
	cout<<"****INSERTAR personaje****"<<endl;
	cout<<"**************************"<<endl<<endl;
	aniade_personaje("Cristina",v);
	escribir_arbol_completo();
	cout<<"*************************"<<endl;
	cout<<"****BORRADO personaje****"<<endl;
	cout<<"*************************"<<endl<<endl;
	elimina_personaje("Cristina");
	escribir_arbol_completo();
...
string QuienEsQuien::obtenerString(bintree<Pregunta>::node n){
	if((*n).obtener_num_personajes()>1){
		return (*n).obtener_pregunta();
	}else{
		return (*n).obtener_personaje();
	}
}
...
void QuienEsQuien::elimina_personaje(string nombre){
	...
	bintree<Pregunta>::node nodoPersonaje;
	buscarPersonaje(nodo,nombre,nodoPersonaje);
	bintree<Pregunta> arbol_aux;
	if(!nodoPersonaje.null()){//Si no encuentra el nodo no hacemos nada
		if(nodoPersonaje != arbol.root()){
			if(nodoPersonaje.parent().right() ==  nodoPersonaje){//si cuelgo por la derecha
				arbol.prune_right(nodoPersonaje.parent(),arbol_aux);
				eliminar_nodos_redundantes();//Volvemos a quitar los nodos redundantes
			}else if(nodoPersonaje.parent().left() ==  nodoPersonaje){//si cuelgo por la izquierda
				arbol.prune_left(nodoPersonaje.parent(),arbol_aux);
...
void QuienEsQuien::buscarPersonaje(bintree<Pregunta>::node n, 
const string & nombre,
bintree<Pregunta>::node & nodoPersonaje){
	if(!n.null()){
		if(esHoja(n) && (obtenerString(n) == nombre)){
			nodoPersonaje=n;//guardamos el nodo que buscamos
			bintree<Pregunta>::node stop;//creamos nodo nulo y paramos el recorrido
			buscarPersonaje(stop,nombre,nodoPersonaje);
			buscarPersonaje(stop,nombre,nodoPersonaje);
		}else{
			buscarPersonaje(n.left(),nombre,nodoPersonaje);
			buscarPersonaje(n.right(),nombre,nodoPersonaje);
		}
	}
}
...
void QuienEsQuien::aniade_personaje(string nombre,vector<bool> caracteristicas){
	...
	bintree<Pregunta> arbol_aux;
	bool ultima_car;
	int tam_vec_caract=(caracteristicas.size()-1);
	int ultima_pos_valida=0;
	
	//1. Llegamos a la hoja valida
	for(int i=0;i<tam_vec_caract && !esHoja(n);i++){
		//Si llegamos a una hoja no podemos seguir descendiendo
	...
	//2. Segun el tamanio del vector atributos<->caracteristicas corregimos la posicion del atributo
	//Creamos los objetos, guardamos una copia del padre del nodo actual y creamos un arbol con la nueva caracteristica como raiz.
	//el vector caracteristicas es el mismo que el de atributos. VA A TENER SIEMPRE EL MISMO NUMERO DE ELEMENTOS.
	if((ultima_pos_valida+1)<tam_vec_caract){
		ultima_pos_valida++;
	}
	
	Pregunta pr(atributos[ultima_pos_valida],2);//Creamos la pregunta
	Pregunta per(nombre,1);//Creamos el personaje
	ultima_car=caracteristicas[ultima_pos_valida];//guardamos la ultima caracteristica (segun apunte la var. "ultima_pos_valida")
	n_aux=n.parent();//guardamos el padre para usarlo en las inserciones
	bintree<Pregunta> arbol_aux2(pr);//Creamos un abb con el nuevo nodo pregunta
	
	//3.Determinamos la posicion e insertamos
	if(n.parent().right() ==  n){//si cuelgo por la derecha
		...
		if(ultima_car){
			...
			arbol.insert_right(n_aux,arbol_aux2);
		}else{
		...
		arbol.prune_left(n.parent(),arbol_aux);
		if(ultima_car){
			arbol_aux2.insert_right(arbol_aux2.root(),(*(arbol_aux.root())));
			arbol_aux2.insert_left(arbol_aux2.root(),per);
			arbol.insert_left(n_aux,arbol_aux2);
		}else{
			arbol_aux2.insert_left(arbol_aux2.root(),(*(arbol_aux.root())));
			arbol_aux2.insert_right(arbol_aux2.root(),per);
			arbol.insert_left(n_aux,arbol_aux2);
