...
{
  int id_ten_izq = (id+1)              % num_procesos, //id. tenedor izq. (ssiendo tu el filosofo)
      id_ten_der = (id+num_procesos-1) % num_procesos; //id. tenedor der.
  int peticion;

  while ( true )
  {
    if(id == 0){  //el primer filosofo coge primero el tenedor derecho
                  //y seguidamente coge el izquierdo

      cout <<"Filósofo " <<id <<" solicita ten. der." <<id_ten_der <<endl;
      // ... solicitar tenedor derecho (completar)
      MPI_Ssend(&peticion, 0, MPI_INT, id_ten_der, ETIQ_COGE_TEN, MPI_COMM_WORLD );
      cout <<"Filósofo " <<id << " coge ten. izq." <<id_ten_izq <<endl;
    ...
    //cuando tiene los tenedores empieza a comer
