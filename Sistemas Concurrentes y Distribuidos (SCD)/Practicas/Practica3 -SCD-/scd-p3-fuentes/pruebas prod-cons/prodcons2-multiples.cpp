...
const int
   ...
   ETIQ_CONSUMIDOR         = 6, //las etiquetas no son el numero de orden del proceso
   ETIQ_PRODUCTOR          = 4, //sino el identificador de cada rol
...
void funcion_productor(int proc_prod)
{
  ...
  //cada productor produce num_items/num_prods
   for ( unsigned int i= 0 ; i < num_items/num_prods; i++ )//cada productor produce 4 elementos (*5 productores)
   {
      //producir
      valor = producir();
      //enviar valor
...
*/
   for( unsigned int i=0 ; i < num_items*2; i++ )  //ya que se producen 20 y se consumen 20
   {
      ...
      if ( num_celdas_ocupadas == 0 )// si buffer vacío, el consumidor no puede consumir
         ...
         id_emisor_aceptable = 1 ;      // $~~~$ solo cons.
          ...
          //if(estado.MPI_TAG < ETIQ_BUFFER)        //recibiremos del productor
      ...
      switch (id_emisor_aceptable) {
        ...
        case 1:{//enviamos al consumidor
          ...
          MPI_Ssend(&buffer[num_celdas_ocupadas-1], 1, MPI_INT, estado.MPI_SOURCE, estado.MPI_TAG, MPI_COMM_WORLD);
