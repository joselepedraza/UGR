...
void  funcion_hebra_productora(  )
{
   for( unsigned i = 0 ; i < num_items ; i++ )
   {
	...
	sem_wait(produce);	//si se esta consumiendo, esperamos (inicializado a 1, decrementa y pasa)
		...
		if(c_prod >= tam_vec){	//si llegamos al final volvemos al principio (donde ya se ha consumido en teoria)		---ó 	c_prod = (c_prod+1)%tam_vec
			c_prod = 0;
		}
...
void funcion_hebra_consumidora(  )
{
   for( unsigned i = 0 ; i < num_items ; i++ )
   {
	...
	sem_wait(consume);	//esperar a que el productor produzca para consumir 
		...
		if(c_cons >= tam_vec){	//si se consumiera el ultimo, volveriamos al principio(donde ya se ha producido en teoria)	---ó c_cons = (c_cons+1)%tam_vec
			c_cons = 0;
		}
		//sem_signal(sec_crit);
