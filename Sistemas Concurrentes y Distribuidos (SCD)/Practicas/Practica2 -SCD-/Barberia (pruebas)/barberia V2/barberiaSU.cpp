...
void Barberia::cortarPelo(int num_cliente)
{
  ...
  if(!sillaCorte.empty() /*&& colaBarbero.empty()*/){   //REVISAR (si colaBarbero esta vacia entonces ya está cortando el pelo)
...
void Barberia::siguienteCliente()
{
  ...
  if(!sillaCorte.empty())   //REVISAR
    ...
    colaClientes.signal();    //DUDA [num_cliente] no haria falta una cola para cada cliente no????????????
}
// -----------------------------------------------------------------------------
