...
constexpr int
   ...
   int iprod[5];      //numero de items que consume cada productor
mutex
...
int producir_dato(int num_prod)
{
   ...
   for(int i = 0; i < (num_items/num_prod); i++){ //al ser ambos inclusive, quizas sea con <=
   ...
   mtx.unlock();
   iprod[num_prod] = i;  //actualizamos cuantos items ha producido el productor num_prod
   //contador++;
...
int main()
{
   ...
   thread hebras_prod[5], hebras_cons[5];



   for(int i=0; i<5;i++){//inicializamos las 5 hebras
