...
//SC-> Señalar y continuar: Semantica en la cual el que hace el signal, desbloquea pero no da exclusion mutua al proceso que desbloquea.
// -----------------------------------------------------------------------------
...
int producir_dato(int num_prod)
{
   ...
   mtx.lock();
   for(int i = iprod[num_prod]; i < num_items/num_prod; i++){ //añ ser ambos inclusive, quizas sea con <=
     cout << "producido: " << contador << endl << flush ;
     cout << "producido por productor: " << num_prod << "el numero:" << contador << endl << flush ;
     cont_prod[contador] ++ ; //comprobacion de contadores
     iprod[num_prod]=contador++;  //actualizamos cuantos items ha producido el productor num_prod
   }
   mtx.unlock();

   return iprod[num_prod]; //contador++ ;
}
//----------------------------------------------------------------------
...
void funcion_hebra_productora( ProdCons1SC * monitor, int num_prod )
{
   for( unsigned i = 0 ; i < num_items ; i++ )
   {
...
int main()
{
   ...
   thread hebres_prods[5], hebras_cons[5];

   for(int i=0; i<5;i++){//inicializamos las 3 hebras (una por fumador)
      hebras_prod[i] = thread(funcion_hebra_productora, &monitor);
			hebras_cons[i] = thread(funcion_hebra_consumidora, &monitor);
    }

	for(int i=0; i<5; i++){	//esperando a que finalicen las hebras
		hebras_cons[i].join();
    hebras_prod[i].join();
	}
