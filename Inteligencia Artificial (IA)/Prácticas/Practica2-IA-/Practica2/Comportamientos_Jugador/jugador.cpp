...
bool ComportamientoJugador::pathFinding(const estado &origen, const estado &destino, list<Action> &plan) {	//comportamiento deliberativo (funcion mal implementada, cambiar algoritmo por otro mejor de busqueda)
	//Borro la lista
	plan.clear();
...
Action ComportamientoJugador::think(Sensores sensores) {	//comportamiento reactivo (la accion think esta dentro de un bucle infinito)
  if (sensores.mensajeF != -1){
	...
	// Determinar si tengo que construir un plan (la parte reactiva le dice a la deliberativa que le hace falta un plan)
	if (!hayPlan){
	...
	// Ejecutar el plan
	Action sigAccion;	//lista de acciones del agente reactivo
	if (hayPlan and plan.size()>0){
		sigAccion = plan.front();	//cojo el plan y ejecuto la siguiente accion con front
